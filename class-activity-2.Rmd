...
scores <- rnorm(100, 75, 20)
scores <- pmax(1, pmin(100, scores))

D6 <- data.frame("scores" = round(scores), "group" = sample(c("sport","nature","literature", "music"), 100, replace = TRUE))
...
```{r}
hist(D6$scores, breaks = 40, main = "Performance")
```
...
var <- D6$scores
cut(var, fivenum(var), letters[1:4])
```
...
hist(D6$scores, breaks = 40, col = brewer.pal(10,"Set3"), main = "palette")
```
...
boxplot(D6$scores ~ D6$group, col = col)
```
